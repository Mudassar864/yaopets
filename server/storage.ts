...
export interface IStorage {
  ...
  markUserAsVerified(id: number): Promise<User | undefined>;
  ...
  getFriendsCount(userId: number): Promise<number>; // mutual followers
  isFollowing(followerId: number, followingId: number): Promise<boolean>;
  followUser(followerId: number, followingId: number): Promise<UserRelationship>;
  unfollowUser(followerId: number, followingId: number): Promise<boolean>;
  
  // Pets
  getPet(id: number): Promise<Pet | undefined>;
  getPets(filters?: { status?: string; type?: string; ownerId?: number; city?: string; isActive?: boolean }): Promise<Pet[]>;
  createPet(pet: InsertPet): Promise<Pet>;
  updatePet(id: number, data: Partial<Pet>): Promise<Pet | undefined>;
  
  // Donations
  getDonation(id: number): Promise<Donation | undefined>;
  getDonations(filters?: { type?: string; donorId?: number; city?: string; isActive?: boolean }): Promise<Donation[]>;
  createDonation(donation: InsertDonation): Promise<Donation>;
  updateDonation(id: number, data: Partial<Donation>): Promise<Donation | undefined>;
  
  // Vet Help
  getVetHelp(id: number): Promise<VetHelp | undefined>;
  getVetHelps(filters?: { status?: string; requesterId?: number; vetId?: number; city?: string }): Promise<VetHelp[]>;
  createVetHelp(vetHelp: InsertVetHelp): Promise<VetHelp>;
  updateVetHelp(id: number, data: Partial<VetHelp>): Promise<VetHelp | undefined>;
  
  // Posts
  getPost(id: number): Promise<Post | undefined>;
  getPosts(filters?: { userId?: number; postType?: string; isStory?: boolean; visibilityType?: string }): Promise<Post[]>;
  ...
  getRecentMessages(userId: number): Promise<Message[]>;
  ...
  // Post Interactions
  getPostInteractions(postType: string, postId: number): Promise<PostInteraction[]>;
  getLikeCount(postType: string, postId: number): Promise<number>;
  getComments(postType: string, postId: number): Promise<PostInteraction[]>;
  createPostInteraction(interaction: InsertPostInteraction): Promise<PostInteraction>;
  deletePostInteraction(id: number): Promise<boolean>;
  ...
  getMediaByUrl(url: string): Promise<Media | undefined>;
  getAllMedia(): Promise<Media[]>;
  getMediaByFilename(filename: string): Promise<Media | undefined>;
  createMedia(media: InsertMedia): Promise<Media>;
  updateMedia(id: number, data: Partial<Media>): Promise<Media | undefined>;
  deleteMedia(id: number): Promise<boolean>;
}

// Implementação direta de DatabaseStorage que usa banco de dados PostgreSQL
import { db } from "./db";
...
export class DatabaseStorage implements IStorage {
  ...
  async getPets(filters?: { status?: string; petType?: string; ownerId?: number; city?: string; isActive?: boolean }): Promise<Pet[]> {
    ...
    if (filters) {
      ...
      if (filters.petType) {
        query = query.where(eq(pets.type, filters.petType));
      }
      if (filters.ownerId) {
  ...
  async findConversation(participant1Id: number, participant2Id: number): Promise<Conversation | undefined> {
    return this.getConversationByParticipants(participant1Id, participant2Id);
  }
  ...
  async updateConversationLastMessage(id: number, messageId: number): Promise<Conversation | undefined> {
    const [updated] = await db
      .update(conversations)
      .set({ 
